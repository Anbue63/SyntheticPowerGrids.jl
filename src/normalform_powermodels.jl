...
function make_generator!(dict::Dict{String,Any}, key_b::Int, node::NormalForm)
    ...
    key = length(dict["gen"])
    ((dict["gen"])[string(key)])["pg"] = node.P_set
    ((dict["gen"])[string(key)])["pmin"] = 0.9 * node.P_set
    ((dict["gen"])[string(key)])["pmax"] = 1.1 * node.P_set
    ((dict["gen"])[string(key)])["vg"] = node.V_set
end

function make_bus_ac!(data::Dict{String,Any}, node::NormalForm)
    bus_dict = _make_bus_ac_header(data)
    bus_dict["bus_type"] = 2
    bus_dict["vm"] = abs(node.V_set)  # assumed p.u.
    bus_dict["vmin"] = 0.9 * bus_dict["vm"]
    bus_dict["vmax"] = 1.1 * bus_dict["vm"]
    bus_dict["va"] = angle(node.V_set)
