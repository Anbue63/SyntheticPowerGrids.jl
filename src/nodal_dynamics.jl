...
function get_nodes(pg, op_ancillary, pg_struct)
    ...
    if pg_struct.generation_dynamics == :SchifferApprox 
        nodes = get_nodes_schiffer(pg, op_ancillary, nodes, pg_struct, nodal_dyn_prob)
    elseif pg_struct.generation_dynamics == :Schmietendorf
        nodes = get_nodes_schmietendorf(pg, op_ancillary, nodes, pg_struct, nodal_dyn_prob)
    end
...
function get_nodes_schiffer(pg, op_ancillary, nodes, pg_struct, nodal_dyn_prob)
    ...
    for n in 1:nv(pg.graph)
        if nodal_dyn_prob[n] > 0.5 # Grid Forming
...
function get_nodes_schmietendorf(pg, op_ancillary, nodes, pg_struct, nodal_dyn_prob)
    nodal_parameters = pg_struct.nodal_parameters

    X = nodal_parameters[:X] # 
    α = nodal_parameters[:α] # 
    γ = nodal_parameters[:γ] # 
    
    for n in 1:nv(pg.graph)
        if nodal_dyn_prob[n] > 0.5 # Third Order Machine
            E_set = op_ancillary[n, :v]
            Q_n = op_ancillary[n, :q]
            E_f = E_set + (X * Q_n / (E_set)) # Calculating the field voltage that results in the correct nodal voltage magnitude
